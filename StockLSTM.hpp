...
class NaiveLSTM : public torch::nn::Module
{
  ...
  /**
   * @enum Dim
   * @brief Enumeration to represent batch, sequence and feature part of Pytorch
   * input tensor data.
   */
  enum Dim : int64_t
  {
...
/**
 * @class StockLSTM
 * @brief A class abstracting the Pytorch's LSTM modules and reprsenting the
...
public:
   ...
   * @param [in] dropOutOpts The reference to Pytorch DropoutOptions for the
   * drop out layer.
   * @param [in] linearOpts The reference to Pytorch LinearOptions for the final
   * linear layer.
   */
