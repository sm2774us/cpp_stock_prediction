 ...
 * @brief Manages the predition for future stock values after loading a time
 * series and trained model for desired stock. The front end browser can <ul>
...
public:
   ...
   * @param [in] args The arguments received from the front-end web browser
   * client.
   */
...
private:
  ...
  void loadTimeSeries();
  /**
   * @brief Helper Method to perform prediction for given input set and desired
   * output
   * @param [in] input The normalized input vector for prediction.
   * @param [in] expectedOuput <i>optional</i> The normalized expected output
   * vector (used with validation dataset).
   */
  std::vector<float> predict(const std::vector<float> &input,
                             const std::vector<float> &expectedOuput = {});
  /**
   * @brief Helper Method to log the results into a csv file
   * @param [in] logFileName The log file name used for writing the predicted
   * result.
   * @param [in] y_test The expected data vector written to the file.
   * @param [in] input <i>optional</i> The trained data vector written to the
   * file.
   * @param [in] allDates <i>optional</i> The date vector for which the
   * predictions were made. Not required in case for future dates predictions.
   */
  ...
  MinMaxScaler<float>
      m_minmaxScaler; ///< The MinMaxScaler object for (de)normalizing data
  std::shared_ptr<StockLSTM>
      m_lstmNetwork; ///< The pointer to StockLSTM neural network object
  std::shared_ptr<StockPrices>
      m_stockPrices;         ///< The pointer to StockPrices for processing raw stock
                             ///< values
